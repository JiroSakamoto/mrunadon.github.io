...
```
#extract Samples
#reshape2 を使ってる場合はrstan::extract()で取り出してください。
...
ここからは、まず「検索があるかないか」をべるぬ〜い分布で考えた上で、

「検索があった時に　どの程度注目が集まったのか」考えることができる、ゼロ過剰ポアソン分布(Zero-Inflated Poisson)で推定していきたいと思います。

言い換えますと、確率的に「検索があった日」を切り分け、「検索があった日に平均的にどの程度の人気度があったのか」を、ポアソン分布の平均パラメータで求めていきます。
